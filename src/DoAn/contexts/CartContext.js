...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      if (userEmail) {
        try {
          const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/cart?email=${userEmail}`);
          const cartItems = response.data;

          // Fetch product details for each item in the cart
          const productDetailsPromises = cartItems.map(async (item) => {
            const productResponse = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/san-pham/${item.product_id}`);
            return {
              ...item,
  ...
  const removeFromCart = async (productId) => {
    ...
    try {
      await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/api/cart/${userEmail}/${productId}`);
  ...
  const increaseQuantity = async (productId) => {
    try {
      ...
      setCart(updatedCart);
      await axios.put(`${process.env.REACT_APP_API_BASE_URL}/api/cart/${userEmail}/${productId}`, {
  ...
  const clearCart = async () => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/api/cart/${userEmail}`);
      setCart([]);
    } catch (error) {
