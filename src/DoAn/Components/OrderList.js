...
const OrderList = () => {
    ...
    useEffect(() => {        
        const fetchOrders = async () => {               
            ...
            try {
                const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/don-hang-user?email=${email}`);
                if (!response.ok) {
    ...
    const fetchOrderDetails = async (orderId) => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/don-hang/${orderId}/details`);
            if (!response.ok) {
    ...
    const handleCancelOrder = async (order) => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/don-hang/${order.order_id}/cancel`, {
                method: 'POST',
            });
