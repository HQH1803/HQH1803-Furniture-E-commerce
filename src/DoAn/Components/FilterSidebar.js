...
const FilterSidebar = ({ onFilterChange, showCategories }) => {
  ...
  const handleToggleFilter = () => {
    ...
    // Nếu đang mở filter, scroll nội dung filter về đầu
    if (!isFilterOpen) {
      setTimeout(() => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setIsFilterFixed(currentPosition > 350);
    };
  ...
  return (
    <div className={`filter-sidebar ${isFilterFixed ? 'fixed' : ''}`}>
      <div className="filter-toggle" onClick={handleToggleFilter}>
        <h3>Lọc Sản Phẩm <span className={`toggle-icon ${isFilterOpen ? 'open' : ''}`}>▼</span></h3>
      </div>
            ...
            {selectedCategories.length > 0 && (
              <div>
                ...
                <span>
                  {selectedCategories
                    .map(id => 
                      categories.find(category => category.id === id)?.tenLoaiSP)
                    .filter(Boolean)
                    .join(', ')}
                </span>
              </div>
