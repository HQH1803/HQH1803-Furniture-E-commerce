...
const QuanLyKichThuoc = () => {
  ...
  const handleModalOk = async () => {
    try {
        ...
        if (isEditMode) {
            await axios.put(`${process.env.REACT_APP_API_BASE_URL}/api/kich-thuoc/${editingRecord.id}`, {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/api/kich-thuoc/${id}`);
