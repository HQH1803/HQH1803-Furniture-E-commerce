...
const QuanLyLoaiPhong = () => {
  ...
  const handleDelete = async (roomTypeId) => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/api/loai-phong/${roomTypeId}`);
  ...
  const handleOk = async (values) => {
    ...
    try {
      if (currentRoomType) {
        // Edit mode
        await axios.put(`${process.env.REACT_APP_API_BASE_URL}/api/loai-phong/${currentRoomType.id}`, values);
