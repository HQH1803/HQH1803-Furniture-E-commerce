...
const QuanLyLoaiSP = () => {
  ...
  const handleOk = async (values) => {
    try {
      if (isEditing) {
        // Edit mode
        await axios.put(`${process.env.REACT_APP_API_BASE_URL}/api/loai-san-pham/${currentProductType.id}`, values);
        ...
        // Add mode
        const response = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/api/loai-san-pham`, values);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/api/loai-san-pham/${id}`);
